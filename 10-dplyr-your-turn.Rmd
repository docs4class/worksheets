...
```{marginfigure}
Can you tell what's going on with this code?  I hope so because we (you!) will leverage these ideas!
```
...
Warm up!
Take a look at an overview of the data with the `skim()` function.  Please note that  `head()`, `glimpse()`, `str()`, `view()`and `summary()` could also be helpful.  **I tend to use the RStudio environment view the most. Let's make sure you feel comfortable with the blue arrow in the RStudio environment view.**
